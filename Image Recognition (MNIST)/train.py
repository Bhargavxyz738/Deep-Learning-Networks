...
base_train_dataset_no_aug = torchvision.datasets.MNIST(root='./data', train=True, download=True, transform=test_val_transform)
...
base_valid_subset = Subset(base_train_dataset_no_aug, valid_idx)

train_dataset = MultiDigitVariableMNIST(base_train_subset, num_samples=54000, max_digits=CONFIG["max_digits"])
...
for epoch in range(CONFIG["epochs"]):
    ...
    if (epoch + 1) % 5 == 0 and (epoch + 1) < CONFIG["epochs"]:
        ...
        if user_input not in ['y', 'yes']:
            ...
            torch.save(model.state_dict(), manual_stop_model_path)
